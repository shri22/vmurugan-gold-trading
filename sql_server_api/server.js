...
const axios = require('axios'); // ADDED: For external API calls (Omniware)
...
// CORS configuration - USES ENV VARIABLE OR MASTER DOMAIN
const MASTER_DOMAIN = process.env.DOMAIN_NAME || 'prodapi.vmuruganjewellery.co.in';
const ALLOWED_ORIGINS = process.env.ALLOWED_ORIGINS
  ? process.env.ALLOWED_ORIGINS.split(',').map(origin => origin.trim())
  : [
    `https://${MASTER_DOMAIN}`,
    `https://${MASTER_DOMAIN}:443`,
    `https://${MASTER_DOMAIN}:3001`,
    'http://localhost:3000',
    'http://localhost:3001'
  ];
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    writeServerLog(`🚫 CORS: Blocked request from unauthorized origin: ${origin}`, 'security');

    // For development, allow all origins (controlled by env variable)
  ...
  allowedHeaders: ['Content-Type', 'Accept', 'Origin', 'User-Agent', 'Authorization', 'admin-token', 'X-Requested-With', 'X-Signature', 'X-Timestamp', 'X-Customer-Id'],
  optionsSuccessStatus: 200
}));

// Rate limiting - Increased for admin portal
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 500, // Increased from 100 to handle admin portal data loading
...
const JWT_SECRET = process.env.JWT_SECRET || 'VMURUGAN_JWT_SECRET_CHANGE_IN_PRODUCTION_2025';
const JWT_EXPIRATION = process.env.JWT_EXPIRATION || '7d'; // Extended to 7 days for payment flows
const ADMIN_USERNAME = process.env.ADMIN_USERNAME || 'admin';
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'Admin@2025';
const ADMIN_TOKEN = process.env.ADMIN_TOKEN || 'VMURUGAN_ADMIN_2025';
const HMAC_SECRET = process.env.HMAC_SECRET || 'VMURUGAN_HMAC_SECRET_CHANGE_IN_PRODUCTION_2025';
const HMAC_TIMESTAMP_TOLERANCE = parseInt(process.env.HMAC_TIMESTAMP_TOLERANCE) || 300; // 5 minutes

// Debug: Log the loaded ADMIN_TOKEN
console.log('🔐 ADMIN_TOKEN loaded:', ADMIN_TOKEN);
console.log('🔐 ADMIN_TOKEN length:', ADMIN_TOKEN.length);
console.log('🔐 ADMIN_TOKEN from env?', process.env.ADMIN_TOKEN ? 'YES' : 'NO (using default)');
...
/**
 * Normalize phone number for comparison and search
 * Removes all non-numeric characters and returns last 10 digits for Indian numbers
 */
...
function generateHMACSignature(data, timestamp, secret = HMAC_SECRET) {
  const message = JSON.stringify(data) + timestamp;
  return crypto.createHmac('sha256', secret).update(message).digest('hex');
}

function verifyHMACSignature(data, timestamp, signature, secret = HMAC_SECRET) {
  const expectedSignature = generateHMACSignature(data, timestamp, secret);
...
// Middleware: Admin Authentication (JWT or Static Token)
function authenticateAdmin(req, res, next) {
  try {
    const authHeader = req.headers.authorization;
    const adminToken = req.headers['admin-token'];

    // Check JWT token first (preferred method)
    ...
    // Fallback to static admin-token (backward compatibility)
    ...
    writeServerLog(`🚫 Unauthorized admin access attempt from IP: ${req.ip}`, 'security');
    ...
    writeServerLog(`❌ Admin authentication error: ${error.message}`, 'security');
...
// Middleware: Customer Authentication (JWT Token)
function authenticateCustomer(req, res, next) {
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      const token = authHeader.substring(7);
      const decoded = verifyAdminToken(token); // Uses same JWT_SECRET
    ...
    // No valid authentication found
    if (!authHeader) {
      writeServerLog(`🚫 Unauthorized: Missing Authorization header from IP: ${req.ip}`, 'security');
    } else if (!authHeader.startsWith('Bearer ')) {
      writeServerLog(`🚫 Unauthorized: Invalid Authorization format from IP: ${req.ip}`, 'security');
    } else {
      writeServerLog(`🚫 Unauthorized: Invalid or expired token from IP: ${req.ip}`, 'security');
    }
    ...
    writeServerLog(`❌ Customer authentication error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Authentication error',
      message: error.message
...
// Middleware: Optional Customer Authentication (doesn't fail if no token)
...
// Middleware: Verify Scheme Ownership
...
// Middleware: Flexible Authentication (JWT OR Admin Token)
// Used for transaction creation where payment gateway already verified payment
function flexibleAuth(req, res, next) {
  try {
    const adminToken = req.headers['admin-token'];
    const authHeader = req.headers['authorization'];

    console.log('🔐 flexibleAuth called');
    console.log('   Received admin-token:', adminToken);
    console.log('   Expected ADMIN_TOKEN:', ADMIN_TOKEN);
    console.log('   Tokens match?', adminToken === ADMIN_TOKEN);
    console.log('   adminToken length:', adminToken ? adminToken.length : 'null');
    console.log('   ADMIN_TOKEN length:', ADMIN_TOKEN.length);

    // Option 1: Check admin token first (always works)
    if (adminToken === ADMIN_TOKEN) {
      console.log('✅ Request authenticated with admin token');
      req.isAdminAuth = true;
      return next();
    }

    // Option 2: Try JWT authentication
    if (authHeader && authHeader.startsWith('Bearer ')) {
      const token = authHeader.substring(7);
      ...
      try {
        ...
        console.log('✅ Request authenticated with JWT token for customer:', decoded.phone);
        return next();
      } catch (jwtError) {
        console.log('⚠️ JWT verification failed:', jwtError.message);
        // Don't return error yet, try admin token as fallback
      }
    }

    // If neither worked, return error
    console.log('❌ Authentication failed - neither admin token nor JWT valid');
    return res.status(401).json({
      success: false,
      error: 'Unauthorized',
      message: 'Invalid or missing authentication. Please provide either JWT token or admin token.'
    });
...
// Middleware: Verify Phone Ownership
function verifyPhoneOwnership(req, res, next) {
  try {
    ...
    const normalizedTokenPhone = normalizePhone(req.customer.phone);
    const normalizedTargetPhone = normalizePhone(phone);

    if (normalizedTokenPhone !== normalizedTargetPhone) {
      writeServerLog(`🚫 Unauthorized phone access attempt: ${normalizedTokenPhone} tried to access ${normalizedTargetPhone}`, 'security');
      return res.status(403).json({
        success: false,
        error: 'Forbidden',
    ...
    writeServerLog(`❌ Phone ownership verification error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Verification error',
      message: error.message
...
// Middleware: Verify Customer Match in Request Body
function verifyCustomerMatch(req, res, next) {
  try {
    ...
    // Check customer_id if provided
    if (customer_id && customer_id !== req.customer.customer_id) {
      writeServerLog(`🚫 Customer ID mismatch: ${req.customer.customer_id} tried to use ${customer_id}`, 'security');
      return res.status(403).json({
        success: false,
        error: 'Forbidden',
    ...
    if (customer_phone) {
      const normalizedTokenPhone = normalizePhone(req.customer.phone);
      const normalizedBodyPhone = normalizePhone(customer_phone);

      if (normalizedTokenPhone !== normalizedBodyPhone) {
        writeServerLog(`🚫 Phone mismatch: ${normalizedTokenPhone} tried to use ${normalizedBodyPhone}`, 'security');
        return res.status(403).json({
          success: false,
          error: 'Forbidden',
    ...
    writeServerLog(`❌ Customer match verification error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Verification error',
      message: error.message
...
// Middleware: Audit Log for Critical Actions
function auditLog(action) {
  return async (req, res, next) => {
    try {
      const customer_id = req.customer?.customer_id || 'ANONYMOUS';
      const admin_username = req.admin?.username || null;
      const resource_id = req.params.scheme_id || req.params.phone || req.params.transaction_id || null;

      // Log to database (if audit_log table exists)
      try {
        await pool.request()
          .input('action', sql.NVarChar, action)
          .input('customer_id', sql.NVarChar, customer_id)
          .input('admin_username', sql.NVarChar, admin_username)
          ...
          .input('request_body', sql.NVarChar, JSON.stringify(req.body))
          .input('timestamp', sql.DateTime, new Date())
          .query(`
            IF EXISTS (SELECT * FROM sys.tables WHERE name = 'audit_log')
            BEGIN
              INSERT INTO audit_log 
              (action, customer_id, admin_username, resource_id, ip_address, user_agent, request_body, timestamp)
              VALUES (@action, @customer_id, @admin_username, @resource_id, @ip_address, @user_agent, @request_body, @timestamp)
            END
          `);
      } catch (dbError) {
        // If audit_log table doesn't exist, just log to file
        writeServerLog(`📝 AUDIT: ${action} by ${customer_id} on ${resource_id} from ${req.ip}`, 'audit');
      }

      // Always log to file
      writeServerLog(`📝 AUDIT: ${action} | User: ${customer_id} | Resource: ${resource_id} | IP: ${req.ip}`, 'audit');

      next();
    } catch (error) {
      // Don't block request if audit fails
      writeServerLog(`⚠️ Audit log error: ${error.message}`, 'audit');
      next();
    }
  };
}
...
const WORLDLINE_SECRET_KEY = process.env.WORLDLINE_SECRET_KEY || '';
const WORLDLINE_API_URL = process.env.WORLDLINE_API_URL || '';
const WORLDLINE_API_KEY = process.env.WORLDLINE_API_KEY || '';
const OMNIWARE_SECRET_KEY = process.env.OMNIWARE_SECRET_KEY || '';
const PAYMENT_SIGNATURE_SECRET = process.env.PAYMENT_SIGNATURE_SECRET || 'PAYMENT_SECRET_CHANGE_IN_PRODUCTION';
const PAYMENT_GATEWAY_IPS = process.env.PAYMENT_GATEWAY_IPS
  ? process.env.PAYMENT_GATEWAY_IPS.split(',').map(ip => ip.trim())
  : ['127.0.0.1'];

// Middleware: Verify Payment Gateway IP
function verifyPaymentGatewayIP(req, res, next) {
  ...
  try {
    ...
    if (!PAYMENT_GATEWAY_IPS.includes(clientIP)) {
      writeServerLog(`🚫 Payment callback from unauthorized IP: ${clientIP}`, 'security');
      return res.status(403).json({
        success: false,
        error: 'Forbidden',
    ...
    writeServerLog(`❌ Payment IP verification error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Verification error',
      message: error.message
...
function verifyPaymentSignature(req, res, next) {
  if (process.env.ENABLE_PAYMENT_SIGNATURE_VERIFICATION === 'false') {
    return next();
  }

  try {
    const { signature, transaction_id, status, amount } = req.body;

    if (!signature) {
      writeServerLog(`🚫 Payment callback without signature for ${transaction_id}`, 'security');
      return res.status(401).json({
        success: false,
    ...
    const message = `${transaction_id}|${status}|${amount}`;
    ...
    if (signature !== expectedSignature) {
      writeServerLog(`🚫 Invalid payment signature for ${transaction_id}`, 'security');
      return res.status(401).json({
        success: false,
    ...
    writeServerLog(`❌ Payment signature verification error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Verification error',
      message: error.message
...
// Middleware: Verify Transaction Exists and is Pending
async function verifyTransactionPending(req, res, next) {
  try {
    ...
    if (result.recordset.length === 0) {
      writeServerLog(`🚫 Payment callback for non-existent transaction: ${transaction_id}`, 'security');
    ...
    if (transaction.status === 'SUCCESS') {
      writeServerLog(`⚠️ Duplicate payment callback for already successful transaction: ${transaction_id}`, 'security');
      return res.status(400).json({
        success: false,
    ...
    writeServerLog(`❌ Transaction verification error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Verification error',
      message: error.message
...
// Middleware: Verify Payment with Gateway (Server-to-Server)
function verifyWithPaymentGateway(gateway) {
  return async (req, res, next) => {
    try {
      const { transaction_id, gateway_transaction_id } = req.body;

      // Skip if gateway verification is disabled
      if (process.env.ENABLE_GATEWAY_VERIFICATION === 'false') {
        writeServerLog(`⚠️ Gateway verification disabled for ${transaction_id}`, 'security');
        return next();
      }
      ...
      if (gateway === 'worldline') {
        // Verify with Worldline API
        try {
          const axios = require('axios');
          gatewayResponse = await axios.post(
            `${WORLDLINE_API_URL}/verify`,
            {
          ...
          if (gatewayResponse.data.status !== 'SUCCESS') {
            writeServerLog(`🚫 Worldline verification failed for ${transaction_id}: ${gatewayResponse.data.status}`, 'security');
            return res.status(400).json({
              success: false,
          ...
          writeServerLog(`⚠️ Worldline API error for ${transaction_id}: ${gatewayError.message}`, 'security');
          // Continue anyway if gateway API is down (log for manual verification)
        }
      } else if (gateway === 'omniware') {
        // Similar verification for Omniware
        // ... (implement based on Omniware API docs)
      }

      next();
    } catch (error) {
      writeServerLog(`❌ Gateway verification error: ${error.message}`, 'security');
      // Don't block if verification fails - log for manual check
      next();
    }
  };
}
...
// ========================================
// METAL RATE MANAGEMENT FUNCTIONS
// ========================================

// Fetch current metal rate from dynamic source with emergency fallback
async function getCurrentMetalRate(metal_type, clientRate = null) {
  try {
    console.log(`🌐 Scraping live ${metal_type} rate from thejewellersassociation.org...`);

    // In a real environment, we would use a robust scraper. 
    // Here we use fetch with a fallback to the database/defaults.
    const response = await fetch('https://thejewellersassociation.org/', {
      headers: {
    ...
    if (response.ok) {
      ...
      if (metal_type === 'GOLD') {
        // Try multiple patterns for 22Kt Gold
        const goldPatterns = [
          /id="goldrate_22ct"[^>]*>([\d,]+\.?[\d]*)/i,
          /class="gold_rate"[^>]*>([\d,]+\.?[\d]*)/i,
          /goldrate_22ct.*?\.html\(['"]([\d,]+\.?[\d]*)['"]\)/i,
          /22Kt.*?([\d,]+\.?[\d]*)/i
        ];
        ...
        for (const pattern of goldPatterns) {
          ...
          if (match) {
            ...
            console.log(`✅ Found gold rate with pattern ${pattern}`);
            break;
          }
        }
      ...
      if (rate) {
        console.log(`✅ Scraped live ${metal_type} rate: ₹${rate}`);
        return rate;
      } else {
        console.log(`⚠️ Could not parse ${metal_type} rate from HTML. HTML sample: ${html.substring(0, 500)}`);
      }
    } else {
      console.log(`⚠️ Scraping failed for ${metal_type} - HTTP ${response.status}: ${response.statusText}`);
    }

    // STEP 2: VERIFY CLIENT-PROVIDED RATE (Genuineness Check)
    // If the scraper is down, we trust the rate from the Flutter app 
    // but verify it against very broad "Sanity Bounds" to prevent garbage data.

    // Future-proof sanity bounds: Won't break even if gold doubles or triples.
    const sanity = metal_type === 'GOLD' ? { min: 2000, max: 100000 } : { min: 20, max: 5000 };

    if (clientRate) {
      const parsedClientRate = parseFloat(clientRate);

      // Check for genuineness: Must be a valid number and within reasonable human range
      if (!isNaN(parsedClientRate) && parsedClientRate >= sanity.min && parsedClientRate <= sanity.max) {
        console.log(`📡 Scrapper Offline. Using verified Client Rate for ${metal_type}: ₹${parsedClientRate}`);
        writeServerLog(`⚠️ UNVERIFIED_CLIENT_RATE: Used client rate ₹${parsedClientRate} for ${metal_type} as scraper was offline.`, 'warning');
        return parsedClientRate;
      }
    }

    // STEP 3: FINAL FALLBACK (Last Resort DB)
    console.log(`⚠️ Scraper and Client Rate failed/non-genuine. Trying last resort DB check...`);
    try {
      const dbResult = await pool.request().query(`SELECT TOP 1 ${metal_type === 'GOLD' ? 'Gold' : 'Silver'} as rate FROM MJData ORDER BY id DESC`);
      if (dbResult.recordset.length > 0) {
        const dbRate = parseFloat(dbResult.recordset[0].rate);
        if (dbRate >= sanity.min && dbRate <= sanity.max) {
          console.log(`✅ Using historical rate from DB as ultimate fallback: ₹${dbRate}`);
          return dbRate;
        }
      }
    } catch (dbErr) { /* ignore */ }

    throw new Error(`Critical: Could not find a genuine ${metal_type} rate from Scraper or Client.`);

  } catch (error) {
    console.error(`❌ Error in getCurrentMetalRate for ${metal_type}:`, error.message);
    // If we're here, it means everything failed. 
    // To protect the payment, we use the client rate IF it exists (> 0), even if it feels "non-genuine"
    if (clientRate && parseFloat(clientRate) > 0) {
      console.log(`🚨 EMERGENCY ESCALATION: Forcing clientRate ₹${clientRate} to save transaction.`);
      return parseFloat(clientRate);
    }
    throw error;
  }
}

// Update metal rate in database (from MJDATA or admin)
async function updateMetalRate(metal_type, rate, source = 'MJDATA') {
  try {
    // Validate rate is within reasonable bounds
    const bounds = {
      GOLD: { min: 2000, max: 50000 },
      SILVER: { min: 20, max: 2000 }
    };

    const { min, max } = bounds[metal_type];
    if (rate < min || rate > max) {
      console.log(`⚠️ ${metal_type} rate ${rate} out of bounds (${min}-${max}), not updating`);
      return false;
    }
    ...
    await pool.request()
      ...
      .query(`
        INSERT INTO metal_rates (metal_type, rate, source, is_active, updated_at)
        VALUES (@metal_type, @rate, @source, 1, SYSDATETIME())
      `);

    console.log(`✅ Updated ${metal_type} rate to ₹${rate} (source: ${source})`);
    return true;

  } catch (error) {
    console.error(`❌ Error updating ${metal_type} rate:`, error.message);
    return false;
  }
}

// Fetch rates from MJDATA and update database
async function fetchAndUpdateRatesFromMJDATA() {
  try {
    const response = await fetch('https://www.mjdta.com/');
    if (!response.ok) {
      console.log('⚠️ MJDTA returned non-OK status:', response.status);
      return false;
    }

    const html = await response.text();

    // Parse gold price (22K)
    const goldMatch = html.match(/22\s*K.*?₹\s*([\d,]+)/i);
    if (goldMatch) {
      const goldPrice = parseFloat(goldMatch[1].replace(/,/g, ''));
      await updateMetalRate('GOLD', goldPrice, 'MJDATA');
    }
    ...
    if (silverMatch) {
      ...
      await updateMetalRate('SILVER', silverPrice, 'MJDATA');
    }

    return true;

  } catch (error) {
    console.error('❌ Error fetching rates from MJDATA:', error.message);
    return false;
  }
}

// Server-side calculation of metal grams
function calculateMetalGrams(amount, current_rate) {
  if (!amount || !current_rate || amount <= 0 || current_rate <= 0) {
    throw new Error('Invalid amount or rate for calculation');
  }

  const metal_grams = parseFloat((amount / current_rate).toFixed(4));

  // Sanity check
  if (metal_grams <= 0 || metal_grams > 10000) {
    throw new Error(`Calculated metal grams (${metal_grams}) out of reasonable range`);
  }

  return metal_grams;
}

// Verify client's calculation matches server's calculation
function verifyCalculation(amount, client_metal_grams, client_rate, server_rate) {
  const tolerance = 0.01; // Allow 1% difference for rate variations

  // Calculate what grams should be based on server rate
  const server_metal_grams = calculateMetalGrams(amount, server_rate);

  // Check if client's grams are within tolerance
  const difference = Math.abs(server_metal_grams - client_metal_grams);
  const percentDiff = (difference / server_metal_grams) * 100;

  if (percentDiff > tolerance) {
    throw new Error(
      `Calculation mismatch: Server calculated ${server_metal_grams} grams ` +
      `(rate: ₹${server_rate}), but client sent ${client_metal_grams} grams ` +
      `(rate: ₹${client_rate}). Difference: ${percentDiff.toFixed(2)}%`
    );
  }
...
// Note: OTP is handled by Firebase Authentication
// Firebase sends OTP via SMS and validates it
// No need for custom OTP storage in database

// Middleware: HMAC Request Signature Validation
function validateHMACSignature(req, res, next) {
  // Skip HMAC validation if disabled or for admin routes
  if (process.env.ENABLE_HMAC_VALIDATION === 'false' || req.path.includes('/admin')) {
    return next();
  }
  ...
  try {
    ...
    // Skip validation for public endpoints and auth endpoints
    ...
    // If user has valid JWT token, skip HMAC validation
    if (req.customer && req.customer.customer_id) {
      return next();
    }

    // Require signature for protected endpoints without JWT
    if (!signature || !timestamp) {
      writeServerLog(`🚫 Missing HMAC signature from IP: ${req.ip}`, 'security');
      return res.status(401).json({
        success: false,
        error: 'Missing signature',
        message: 'Request signature or authentication token required'
      });
    }
    ...
    if (Math.abs(now - requestTime) > HMAC_TIMESTAMP_TOLERANCE) {
      writeServerLog(`🚫 Expired request signature from IP: ${req.ip}`, 'security');
      return res.status(401).json({
        success: false,
    ...
    try {
      if (!verifyHMACSignature(requestData, timestamp, signature)) {
        writeServerLog(`🚫 Invalid HMAC signature from IP: ${req.ip}, Customer: ${customerId}`, 'security');
        return res.status(401).json({
          success: false,
          error: 'Invalid signature',
      ...
      writeServerLog(`🚫 HMAC verification error: ${error.message}`, 'security');
      return res.status(401).json({
        success: false,
        error: 'Invalid signature',
        message: 'Request signature verification failed'
    ...
    // Signature valid
    req.verifiedCustomer = customerId;
    next();
  } catch (error) {
    writeServerLog(`❌ HMAC validation error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
...
const otpLimiter = rateLimit({
  windowMs: parseInt(process.env.OTP_RATE_LIMIT_WINDOW_MS) || 300000, // 5 minutes
...
async function createTablesIfNotExist() {
  try {
    ...
    await pool.request().query(`
      ...
      BEGIN
        CREATE TABLE customers (
          ...
          terms_accepted_ip NVARCHAR(45) NULL,
          ...
          total_silver DECIMAL(10,4) DEFAULT 0.0000,
    ...
    // Add Terms & Conditions columns if they don't exist
    await pool.request().query(`
      ...
      BEGIN
        ...
        PRINT 'Added terms_accepted_ip column'
      END
    `);
    ...
    await pool.request().query(`
      ...
      BEGIN
        CREATE TABLE transactions (
          ...
          gold_grams DECIMAL(10,4) NOT NULL DEFAULT 0.0000,
          gold_price_per_gram DECIMAL(10,2) NOT NULL DEFAULT 0.00,
          ...
          payment_year INT,
          payment_month INT,
          metal_type NVARCHAR(10) NULL CHECK (metal_type IN ('GOLD', 'SILVER', NULL)),
    ...
    // Add default values to gold_grams and gold_price_per_gram if they don't have them
    await pool.request().query(`
      IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('transactions') AND name = 'gold_grams')
      BEGIN
        IF NOT EXISTS (SELECT * FROM sys.default_constraints WHERE parent_object_id = OBJECT_ID('transactions') AND parent_column_id = COLUMNPROPERTY(OBJECT_ID('transactions'), 'gold_grams', 'ColumnId'))
        BEGIN
          ALTER TABLE transactions ADD CONSTRAINT DF_transactions_gold_grams DEFAULT 0.0000 FOR gold_grams
          PRINT 'Added default value to gold_grams in transactions table'
        END
      END

      IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('transactions') AND name = 'gold_price_per_gram')
      BEGIN
        IF NOT EXISTS (SELECT * FROM sys.default_constraints WHERE parent_object_id = OBJECT_ID('transactions') AND parent_column_id = COLUMNPROPERTY(OBJECT_ID('transactions'), 'gold_price_per_gram', 'ColumnId'))
        BEGIN
          ALTER TABLE transactions ADD CONSTRAINT DF_transactions_gold_price DEFAULT 0.00 FOR gold_price_per_gram
          PRINT 'Added default value to gold_price_per_gram in transactions table'
        END
      END
    `);
    ...
    await pool.request().query(`
      ...
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('transactions') AND name = 'gateway_response')
      BEGIN
        ALTER TABLE transactions ADD gateway_response NVARCHAR(MAX)
        PRINT 'Added gateway_response column to transactions table'
      END
    `);
    ...
    // Add metal_type column to existing schemes table if it doesn't exist
    await pool.request().query(`
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'metal_type')
      BEGIN
        ALTER TABLE schemes ADD metal_type NVARCHAR(10) NULL
        PRINT 'Added metal_type column to schemes table'
        
        -- Default existing schemes based on their type
        EXEC('UPDATE schemes SET metal_type = ''GOLD'' WHERE scheme_type LIKE ''%GOLD%''')
        EXEC('UPDATE schemes SET metal_type = ''SILVER'' WHERE scheme_type LIKE ''%SILVER%''')
        
        -- If any still null, default to GOLD
        EXEC('UPDATE schemes SET metal_type = ''GOLD'' WHERE metal_type IS NULL')
      END
    `);
    ...
    // Add closure_remarks column to schemes table if it doesn't exist
    await pool.request().query(`
    ...
    // Add closure_date column to schemes table if it doesn't exist
    await pool.request().query(`
    ...
    await pool.request().query(`
      ...
      BEGIN
        ALTER TABLE transactions ADD payment_year INT
        PRINT 'Added payment_year column to transactions table'
      END
    `);

    // Add payment_month column to existing transactions table if it doesn't exist
    await pool.request().query(`
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('transactions') AND name = 'payment_month')
      BEGIN
        ALTER TABLE transactions ADD payment_month INT
        PRINT 'Added payment_month column to transactions table'
      END
    `);

    // Database migration: Add summary columns to customers if they don't exist
    await pool.request().query(`
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'total_invested')
      BEGIN
        ALTER TABLE customers ADD total_invested DECIMAL(12,2) DEFAULT 0.00
        PRINT 'Added total_invested to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'total_gold')
      BEGIN
        ALTER TABLE customers ADD total_gold DECIMAL(10,4) DEFAULT 0.0000
        PRINT 'Added total_gold to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'total_silver')
      BEGIN
        ALTER TABLE customers ADD total_silver DECIMAL(10,4) DEFAULT 0.0000
        PRINT 'Added total_silver to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'transaction_count')
      BEGIN
        ALTER TABLE customers ADD transaction_count INT DEFAULT 0
        PRINT 'Added transaction_count to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'last_transaction')
      BEGIN
        ALTER TABLE customers ADD last_transaction DATETIME2(3) NULL
        PRINT 'Added last_transaction to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'business_id')
      BEGIN
        ALTER TABLE customers ADD business_id NVARCHAR(50) DEFAULT 'VMURUGAN_001'
        PRINT 'Added business_id to customers'
      END
    `);

    // Database migration: Add tracking columns to customers if they don't exist
    await pool.request().query(`
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'created_at')
      BEGIN
        ALTER TABLE customers ADD created_at DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added created_at to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'updated_at')
      BEGIN
        ALTER TABLE customers ADD updated_at DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added updated_at to customers'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('customers') AND name = 'registration_date')
      BEGIN
        ALTER TABLE customers ADD registration_date DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added registration_date to customers'
      END
    `);

    // Database migration: Add tracking columns to transactions if they don't exist
    await pool.request().query(`
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('transactions') AND name = 'created_at')
      BEGIN
        ALTER TABLE transactions ADD created_at DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added created_at to transactions'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('transactions') AND name = 'updated_at')
      BEGIN
        ALTER TABLE transactions ADD updated_at DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added updated_at to transactions'
      END
    `);

    // Database migration: Add tracking and summary columns to schemes if they don't exist
    await pool.request().query(`
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'total_invested')
      BEGIN
        ALTER TABLE schemes ADD total_invested DECIMAL(12,2) DEFAULT 0.00
        PRINT 'Added total_invested to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'total_metal_accumulated')
      BEGIN
        ALTER TABLE schemes ADD total_metal_accumulated DECIMAL(10,4) DEFAULT 0.0000
        PRINT 'Added total_metal_accumulated to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'completed_installments')
      BEGIN
        ALTER TABLE schemes ADD completed_installments INT DEFAULT 0
        PRINT 'Added completed_installments to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'total_amount_paid')
      BEGIN
        ALTER TABLE schemes ADD total_amount_paid DECIMAL(12,2) DEFAULT 0.00
        PRINT 'Added total_amount_paid to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'duration_months')
      BEGIN
        ALTER TABLE schemes ADD duration_months INT NULL
        PRINT 'Added duration_months to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'customer_id')
      BEGIN
        ALTER TABLE schemes ADD customer_id NVARCHAR(20) NOT NULL DEFAULT ''
        PRINT 'Added customer_id to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'transaction_id')
      BEGIN
        ALTER TABLE schemes ADD transaction_id NVARCHAR(100)
        PRINT 'Added transaction_id to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'business_id')
      BEGIN
        ALTER TABLE schemes ADD business_id NVARCHAR(50) DEFAULT 'VMURUGAN_001'
        PRINT 'Added business_id to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'created_at')
      BEGIN
        ALTER TABLE schemes ADD created_at DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added created_at to schemes'
      END
      IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'updated_at')
      BEGIN
        ALTER TABLE schemes ADD updated_at DATETIME2(3) DEFAULT SYSDATETIME()
        PRINT 'Added updated_at to schemes'
      END
    `);

    // Database migration: Add MPIN column to customers if it doesn't exist
    await pool.request().query(`
    ...
    // Database migration: Add customer_id column to customers if it doesn't exist
    await pool.request().query(`
    ...
    // Ensure customer_id in schemes is NVARCHAR(20) to match customers table
    await pool.request().query(`
      IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('schemes') AND name = 'customer_id' AND TYPE_NAME(system_type_id) = 'int')
      BEGIN
        ALTER TABLE schemes ALTER COLUMN customer_id NVARCHAR(20) NOT NULL
        PRINT 'Altered schemes.customer_id from INT to NVARCHAR(20)'
      END
    `);
...
app.post('/api/admin/login', adminLoginLimiter, async (req, res) => {
  try {
    ...
    writeServerLog(`🔐 Admin login attempt for username: ${username} from IP: ${req.ip}`, 'security');
    ...
    if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
      ...
      writeServerLog(`✅ Admin login successful for: ${username}`, 'security');
    ...
    // Invalid credentials
    writeServerLog(`🚫 Failed admin login attempt for: ${username} from IP: ${req.ip}`, 'security');
    ...
    writeServerLog(`❌ Admin login error: ${error.message}`, 'security');
    return res.status(500).json({
      success: false,
      error: 'Login error',
...
// ========================================
// APP CONFIGURATION & VERSIONING (Force Update)
// ========================================
                ...
                if (typeof $.pnCheckout === 'function') {
                    ...
                    var checkoutData = {
                        ...
                        "consumerData": {
                            ...
                            "token": "${token}",
                            "returnUrl": "https://prodapi.vmuruganjewellery.co.in/api/payments/worldline/verify",
...
app.post('/api/customers', [
  ...
  try {
    ...
    request.input('ip_address', sql.NVarChar(45), req.ip || req.connection.remoteAddress);

    const normalizedPhone = normalizePhone(phone);
    request.input('normalized_phone', sql.NVarChar(15), normalizedPhone);

    await request.query(`
      IF NOT EXISTS (SELECT 1 FROM customers WHERE phone = @phone OR RIGHT(phone, 10) = @normalized_phone)
      BEGIN
        ...
        INSERT INTO customers (customer_id, phone, name, email, address, pan_card, device_id, business_id, mpin, terms_accepted, terms_accepted_at, terms_accepted_ip)
        VALUES (@customerId, @phone, @name, @email, @address, @pan_card, @device_id, @business_id, @mpin, 1, SYSDATETIME(), @ip_address)
      END
      ELSE
      BEGIN
        UPDATE customers
        SET name = @name, email = @email, address = @address,
            ...
            terms_accepted = 1, terms_accepted_at = SYSDATETIME(), terms_accepted_ip = @ip_address,
            ${hashedMPIN ? 'mpin = @mpin,' : ''} updated_at = SYSDATETIME()
        WHERE phone = @phone OR RIGHT(phone, 10) = @normalized_phone
      END
    `);
...
app.post('/api/auth/send-otp', otpLimiter, [
  ...
  try {
    ...
    // Firebase will send OTP via SMS
    // Client will receive OTP and verify with Firebase
...
app.post('/api/auth/verify-otp', otpLimiter, [
  ...
  try {
    ...
    // Firebase handles OTP validation on the client side
    // If client reaches here, Firebase has already validated the OTP
    // We just need to issue a JWT token for backend API access
    ...
    writeServerLog(`✅ Customer authenticated via Firebase OTP: ${phone}`, 'security');
...
app.get('/api/customers/:phone', optionalCustomerAuth, async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    res.json({
      ...
      user: {
        ...
        pan_card: customer.pan_card,
        registration_date: customer.registration_date,
        terms_accepted: customer.terms_accepted === true || customer.terms_accepted === 1,
...
// MPIN rate limiter - prevent brute force
const mpinLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 3, // 3 attempts per 15 minutes
  message: 'Too many MPIN attempts. Please try again after 15 minutes.',
  keyGenerator: (req) => req.params.phone, // Per phone number
  standardHeaders: true,
  legacyHeaders: false,
});

// Weak MPIN validation
function validateMPINStrength(mpin) {
  // Check if MPIN is 4 digits
  if (!/^\d{4}$/.test(mpin)) {
    return { valid: false, error: 'MPIN must be exactly 4 digits' };
  }
  ...
  if (weakMPINs.includes(mpin)) {
    return { valid: false, error: 'MPIN too weak. Avoid sequential or repeated digits.' };
  }

  // Check for all same digits
  if (/^(\d)\1{3}$/.test(mpin)) {
    return { valid: false, error: 'MPIN cannot have all same digits' };
  }
  ...
  if (isSequential) {
    return { valid: false, error: 'MPIN cannot be sequential digits' };
  }

  return { valid: true };
}

// Update MPIN endpoint with rate limiting and complexity check
app.post('/api/customers/:phone/update-mpin', authenticateCustomer, verifyPhoneOwnership, mpinLimiter, auditLog('UPDATE_MPIN'), [
  body('current_mpin').isLength({ min: 4, max: 4 }).withMessage('Current MPIN must be 4 digits'),
  body('new_mpin').isLength({ min: 4, max: 4 }).withMessage('New MPIN must be 4 digits').custom((value) => {
    const validation = validateMPINStrength(value);
    if (!validation.valid) {
...
app.post('/api/customers/:phone/set-mpin', authenticateCustomer, verifyPhoneOwnership, mpinLimiter, auditLog('SET_MPIN'), [
  body('new_mpin').isLength({ min: 4, max: 4 }).withMessage('MPIN must be 4 digits').custom((value) => {
    const validation = validateMPINStrength(value);
    if (!validation.valid) {
      throw new Error(validation.error);
    }
...
// Accept Terms & Conditions endpoint
app.post('/api/customers/:phone/accept-terms', authenticateCustomer, verifyPhoneOwnership, auditLog('ACCEPT_TERMS'), async (req, res) => {
  try {
    const { phone } = req.params;
    const clientIP = req.ip || req.connection.remoteAddress;
    ...
    request.input('phone', sql.NVarChar(15), phone);
    request.input('normalized_phone', sql.NVarChar(15), normalizedPhone);
    request.input('ip', sql.NVarChar(45), clientIP);

    await request.query(`
      UPDATE customers 
      SET terms_accepted = 1, 
          terms_accepted_at = SYSDATETIME(), 
          terms_accepted_ip = @ip,
          updated_at = SYSDATETIME() 
      WHERE phone = @phone OR RIGHT(phone, 10) = @normalized_phone
    `);

    console.log('✅ Terms accepted successfully:', phone);
    res.json({
      success: true,
...
// Save transaction
// Save transaction - Flexible auth (JWT OR admin token)
// Payment gateway already verified payment, so we accept admin token as fallback
app.post('/api/transactions', flexibleAuth, auditLog('CREATE_TRANSACTION'), [
  ...
  // Allow status but it will be set to SUCCESS only if verified or if we trust the client (for now)
], async (req, res) => {
  try {
    ...
    // ========================================
    // SERVER-SIDE CALCULATION (SECURITY FIX)
    // ========================================
    let final_gold_grams = gold_grams || 0;
    let final_gold_price = gold_price_per_gram || 0;
    let final_silver_grams = silver_grams || 0;
    let final_silver_price = silver_price_per_gram || 0;

    const now = new Date();
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth() + 1; // 1-based month (1-12)

    // Determine metal type early for calculation
    const metal_type = (silver_grams > 0 || (scheme_type && scheme_type.includes('SILVER'))) ? 'SILVER' : 'GOLD';

    if (type === 'BUY' && amount > 0) {
      console.log(`⚖️ Recalculating ${metal_type} grams server-side for security...`);
      const client_rate = metal_type === 'GOLD' ? gold_price_per_gram : silver_price_per_gram;
      const current_rate = await getCurrentMetalRate(metal_type, client_rate);
      const calculated_grams = calculateMetalGrams(amount, current_rate);

      if (metal_type === 'GOLD') {
        final_gold_grams = calculated_grams;
        final_gold_price = current_rate;
        final_silver_grams = 0;
        final_silver_price = 0;
      } else {
        final_silver_grams = calculated_grams;
        final_silver_price = current_rate;
        final_gold_grams = 0;
        final_gold_price = 0;
      }

      writeServerLog(`🔒 Security: Recalculated ${metal_type} for ${transaction_id}: ₹${amount} -> ${calculated_grams}g (Rate: ₹${current_rate})`, 'security');
    }
    ...
    request.input('gold_grams', sql.Decimal(10, 4), final_gold_grams);
    request.input('gold_price_per_gram', sql.Decimal(10, 2), final_gold_price);
    request.input('silver_grams', sql.Decimal(10, 4), final_silver_grams);
    request.input('silver_price_per_gram', sql.Decimal(10, 2), final_silver_price);
    ...
    request.input('device_info', sql.NVarChar(sql.MAX), device_info || 'Unknown');
    request.input('location', sql.NVarChar(sql.MAX), location || 'Not provided');
    request.input('business_id', sql.NVarChar(50), business_id);
    ...
    request.input('payment_year', sql.Int, currentYear);
    request.input('payment_month', sql.Int, currentMonth);

    // Use the metal_type determined earlier
    request.input('metal_type', sql.NVarChar(10), metal_type);

    // Use MERGE to handle case where transaction was pre-created by /payment-page-url
    // or if the app calls this multiple times (e.g. retry)
    await request.query(`
      MERGE transactions AS target
      USING (SELECT @transaction_id AS transaction_id) AS source
      ON target.transaction_id = source.transaction_id
      WHEN MATCHED AND target.status != 'SUCCESS' THEN
        UPDATE SET 
    ...
    // ========================================
    // INSTANT CREDIT SYSTEM (If Success)
    // ========================================
    if (status === 'SUCCESS') {
      try {
    ...
    res.json({
      ...
      metal_grams: (metal_type === 'GOLD' ? final_gold_grams : final_silver_grams),
      current_rate: (metal_type === 'GOLD' ? final_gold_price : final_silver_price)
    });
...
app.get('/api/transaction-history', async (req, res) => {
  try {
    ...
    console.log(`📊 Fetching transactions for: ${phone} (normalized: ${normalizedPhone})`);

    const result = await request.query(`
      SELECT * FROM transactions
      WHERE (customer_phone = @phone 
             OR RIGHT(customer_phone, 10) = @normalized_phone)
...
app.post('/api/payment-webhook', async (req, res) => {
  try {
    ...
    const alreadySuccess = transaction.status === 'SUCCESS';
...
// ============================================================================
// OMNIWARE GATEWAY CONFIGURATION & SYNC
// ============================================================================
const OMNIWARE_VERIFY_CONFIG = {
  GOLD: {
    merchantId: '779285',
...
/**
 * Generate SHA-512 hash for Omniware status check
 */
...
/**
 * Robustly sync transaction status with Omniware Gateway
 * Checks the gateway for the actual truth of the payment
 */
async function syncTransactionWithOmniware(orderId) {
  try {
    console.log(`🔄 [OMNIWARE-SYNC] Syncing order: ${orderId}`);

    // 1. Determine metal type from Order ID
    let metalType = 'GOLD';
    if (orderId.includes('_SILVER_')) metalType = 'SILVER';

    const config = OMNIWARE_VERIFY_CONFIG[metalType];
    ...
    // 3. Call Omniware Payment Status API
    ...
    const paymentData = Array.isArray(response.data.data) ? response.data.data[0] : response.data.data;
    const responseCode = parseInt(paymentData.response_code);
    const hasValidTime = paymentData.payment_datetime &&
          ...
          try {
            ...
            if (txn.customer_phone && !isRepair) {
              await transaction.request()
                ...
                .input('silver', sql.Decimal(10, 4), txn.silver_grams || 0)
                .input('amt', sql.Decimal(12, 2), txn.amount || 0)
                .query(`
                  UPDATE customers 
    ...
    console.error(`❌ [OMNIWARE-SYNC] Error syncing ${orderId}:`, error.message);
    return null;
  }
}

// Consolidated Verification Endpoint (Gateway-Aware)
  ...
  try {
    ...
    if (currentStatus !== 'SUCCESS' || (!transaction.scheme_id && transaction.customer_phone)) {
      ...
      if (gatewayStatus) {
        ...
        console.log(`🏢 Gateway says final status is: ${currentStatus}`);
      }
    }
    ...
    res.json({
      ...
      needs_update: currentStatus !== transaction.status,
...
// Consolidated Pending Payments Endpoint (Auto-Syncing)
app.get('/api/payment/pending/:phone', flexibleAuth, async (req, res) => {
  try {
    ...
    const result = await pool.request()
      .input('phone', sql.NVarChar, phone)
      .query(`SELECT * FROM transactions WHERE customer_phone = @phone AND (status = 'PENDING' OR (status = 'FAILED' AND created_at > DATEADD(hour, -2, GETDATE()))) ORDER BY created_at DESC`);

    const verified = [];
    for (const txn of result.recordset) {
      let status = txn.status;

      // If it's an Omniware transaction and not success, sync it
      if (status !== 'SUCCESS' && (txn.transaction_id.startsWith('ORD_') || txn.payment_method === 'OMNIWARE_UPI')) {
        const gatewayStatus = await syncTransactionWithOmniware(txn.transaction_id);
        if (gatewayStatus) status = gatewayStatus;
      }

      verified.push({ ...txn, status });
    }

    res.json({ success: true, pending_count: verified.filter(t => t.status === 'PENDING').length, transactions: verified });
...
app.post('/api/schemes/create-after-payment', flexibleAuth, auditLog('CREATE_SCHEME_AFTER_PAYMENT'), [
  body('customer_phone').notEmpty().withMessage('Customer phone required'),
  body('customer_name').notEmpty().withMessage('Customer name required'),
  body('scheme_type').isIn(['GOLDPLUS', 'GOLDFLEXI', 'SILVERPLUS', 'SILVERFLEXI']).withMessage('Invalid scheme type'),
  body('monthly_amount').isFloat({ min: 0 }).withMessage('Monthly amount must be a valid number'),
  ...
  try {
    ...
    // ========================================
    // PAYMENT VERIFICATION (CRITICAL SECURITY)
    // ========================================

    // Step 1: Verify transaction exists
    const txnRequest = pool.request();
    txnRequest.input('transaction_id', sql.NVarChar, transaction_id);
    ...
    if (txnResult.recordset.length === 0) {
      writeServerLog(`🚫 Scheme creation attempted with non-existent transaction: ${transaction_id}`, 'security');
      return res.status(404).json({
        success: false,
        error: 'Transaction not found',
    ...
    // Step 2: Verify transaction belongs to this customer
    if (transaction.customer_phone !== customer_phone) {
      writeServerLog(`🚫 Customer ${customer_phone} tried to use transaction ${transaction_id} belonging to ${transaction.customer_phone}`, 'security');
      return res.status(403).json({
        success: false,
        error: 'Forbidden',
    ...
    if (transaction.status !== 'SUCCESS') {
      writeServerLog(`🚫 Scheme creation attempted with non-successful transaction: ${transaction_id} (status: ${transaction.status})`, 'security');
      return res.status(400).json({
        success: false,
        error: 'Payment not successful',
        message: `Transaction status is ${transaction.status}. Payment must be successful to create scheme.`
      });
    }

    // Step 4: Verify transaction hasn't been used already
    const usedTxnRequest = pool.request();
    usedTxnRequest.input('transaction_id', sql.NVarChar, transaction_id);

    const usedTxnResult = await usedTxnRequest.query(`
      SELECT scheme_id FROM schemes WHERE transaction_id = @transaction_id
    `);

    if (usedTxnResult.recordset.length > 0) {
      const existingSchemeId = usedTxnResult.recordset[0].scheme_id;
      writeServerLog(`🚫 Transaction ${transaction_id} already used for scheme ${existingSchemeId}`, 'security');
      return res.status(400).json({
        success: false,
        error: 'Transaction already used',
        message: `This transaction has already been used to create scheme ${existingSchemeId}`
      });
    }

    // Step 5: Verify amount matches (optional but recommended)
    if (transaction.amount && monthly_amount && Math.abs(transaction.amount - monthly_amount) > 1) {
      writeServerLog(`⚠️ Amount mismatch for transaction ${transaction_id}: txn=${transaction.amount}, scheme=${monthly_amount}`, 'security');
      // Log but don't block - amounts might differ due to fees
    }

    writeServerLog(`✅ Payment verified for scheme creation: ${transaction_id} -> ${customer_phone}`, 'audit');
    ...
    // Create scheme with initial investment data
    const total_metal = metalType === 'GOLD' ? (transaction.gold_grams || 0) : (transaction.silver_grams || 0);
    const schemeRequest = pool.request();
    schemeRequest.input('scheme_id', sql.NVarChar(100), schemeId);
    schemeRequest.input('customer_id', sql.Int, customerId);
    schemeRequest.input('customer_phone', sql.NVarChar(15), customer_phone);
    ...
    schemeRequest.input('end_date', sql.DateTime, endDate);
    schemeRequest.input('business_id', sql.NVarChar(50), 'VMURUGAN_001');
    schemeRequest.input('total_invested', sql.Decimal(12, 2), transaction.amount || monthly_amount);
    schemeRequest.input('total_metal_accumulated', sql.Decimal(10, 4), total_metal);

    schemeRequest.input('transaction_id', sql.NVarChar(100), transaction_id);
    schemeRequest.input('total_amount_paid', sql.Decimal(12, 2), transaction.amount || monthly_amount);

    const schemeResult = await schemeRequest.query(`
      INSERT INTO schemes (
        scheme_id, customer_id, customer_phone, customer_name,
        scheme_type, metal_type, monthly_amount, duration_months,
        total_invested, total_metal_accumulated, total_amount_paid, completed_installments,
        end_date, terms_accepted, terms_accepted_at, business_id, transaction_id
      )
      OUTPUT INSERTED.id
      VALUES (
        @scheme_id, @customer_id, @customer_phone, @customer_name,
        @scheme_type, @metal_type, @monthly_amount, @duration_months,
        @total_invested, @total_metal_accumulated, @total_amount_paid, 1,
        @end_date, 1, SYSDATETIME(), @business_id, @transaction_id
      )
    `);
...
app.get('/api/schemes/:customer_phone', optionalCustomerAuth, async (req, res) => {
  try {
    ...
    schemesRequest.input('normalized_phone', sql.NVarChar(15), normalizedPhone);
    schemesRequest.input('business_id', sql.NVarChar(50), 'VMURUGAN_001');

    console.log(`📊 Fetching schemes for: ${customer_phone} (normalized: ${normalizedPhone})`);

    // Get schemes with calculated values from transactions table
    const schemes = await schemesRequest.query(`
      SELECT
        s.id, s.scheme_id, c.customer_id, s.customer_phone,
    ...
    // Calculate platform-wide portfolio summary from ALL successful transactions
    const portfolioRequest = pool.request();
    // Reuse normalizedPhone from above (line 3351)
    portfolioRequest.input('customer_phone', sql.NVarChar(15), customer_phone);
    portfolioRequest.input('normalized_phone', sql.NVarChar(15), normalizedPhone);
    portfolioRequest.input('business_id', sql.NVarChar(50), 'VMURUGAN_001');

    console.log(`📊 Fetching portfolio for: ${customer_phone} (normalized: ${normalizedPhone})`);
      ...
      SELECT 
        (SELECT COUNT(*) FROM schemes 
         WHERE (customer_phone = @customer_phone 
                OR RIGHT(customer_phone, 10) = @normalized_phone)
           AND status = 'ACTIVE') as total_schemes,
        ISNULL(SUM(amount), 0) as total_invested,
        ISNULL(SUM(gold_grams), 0) as total_gold_grams,
        ISNULL(SUM(silver_grams), 0) as total_silver_grams,
...
app.put('/api/schemes/:scheme_id', authenticateCustomer, verifySchemeOwnership, auditLog('UPDATE_SCHEME'), [
...
app.post('/api/schemes/:scheme_id/invest', flexibleAuth, auditLog('INVEST_SCHEME'), [
  body('amount').isFloat({ min: 10, max: 1000000 }).withMessage('Investment amount must be between ₹10 and ₹10,00,000'),
  body('transaction_id').notEmpty().withMessage('Transaction ID required')
  // ✅ REMOVED: metal_grams (server calculates)
  // ✅ REMOVED: current_rate (server fetches from database)
], async (req, res) => {
  try {
    const { scheme_id } = req.params;
    const { amount, transaction_id, gateway_transaction_id } = req.body;
    ...
    // Step 1: Fetch current metal rate from database (updated from MJDATA)
    const current_rate = await getCurrentMetalRate(scheme.metal_type);
    console.log(`✅ Server fetched ${scheme.metal_type} rate: ₹${current_rate}/gram`);

    // Step 2: Server calculates metal grams (client cannot manipulate)
    const metal_grams = calculateMetalGrams(amount, current_rate);
    console.log(`✅ Server calculated metal grams: ${metal_grams} grams for ₹${amount}`);

    // Step 3: Log calculation for audit
    writeServerLog(
      `💰 Investment calculation: ${scheme.metal_type} - ₹${amount} ÷ ₹${current_rate} = ${metal_grams} grams`,
      'audit'
    );
    ...
    transactionRequest.input('device_info', sql.NVarChar(sql.MAX), req.body.device_info || 'Unknown');
    transactionRequest.input('location', sql.NVarChar(sql.MAX), req.body.location || 'Not provided');
    ...
    transactionRequest.input('payment_year', sql.Int, currentYear);
    transactionRequest.input('payment_month', sql.Int, currentMonth);
...
app.post('/api/schemes/:scheme_id/flexi-payment', flexibleAuth, paymentLimiter, auditLog('FLEXI_PAYMENT'), [
  body('amount').isFloat({ min: 10, max: 1000000 }).withMessage('Payment amount must be between ₹10 and ₹10,00,000'),
  body('transaction_id').notEmpty().withMessage('Transaction ID required')
  // ✅ REMOVED: metal_grams (server calculates)
  // ✅ REMOVED: current_rate (server fetches from database)
], async (req, res) => {
  try {
    const { scheme_id } = req.params;
    const { amount, transaction_id, gateway_transaction_id, payment_method } = req.body;
    ...
    // Step 3: Log calculation for audit
    writeServerLog(
      `💰 Flexi payment calculation: ${scheme.metal_type} - ₹${amount} ÷ ₹${current_rate} = ${metal_grams} grams`,
      'audit'
    );
    ...
    // We use a logic that checks if the transaction already exists (pre-created by Omniware flow)
    // and updates it, or inserts it if it's new.
    await transactionRequest.query(`
      IF EXISTS (SELECT 1 FROM transactions WHERE transaction_id = @transaction_id)
      BEGIN
        UPDATE transactions 
        SET 
          status = @status,
      ...
      BEGIN
        INSERT INTO transactions (
          ...
          device_info, location, payment_year, payment_month, created_at, updated_at
        ) VALUES (
          @transaction_id, @customer_phone, @customer_name, @type, @amount,
          @gold_grams, @gold_price_per_gram, @silver_grams, @silver_price_per_gram,
          @status, @payment_method, @gateway_transaction_id, @business_id,
          @scheme_type, @scheme_id, @metal_type,
          @device_info, @location, @payment_year, @payment_month, GETDATE(), GETDATE()
        )
      END
    `);
...
app.get('/api/schemes/details/:scheme_id', optionalCustomerAuth, async (req, res) => {
  try {
    const { scheme_id } = req.params;
...
app.get('/api/schemes/:scheme_id/payments/monthly-check', optionalCustomerAuth, async (req, res) => {
  try {
    const { scheme_id } = req.params;
...
app.post('/api/payments/worldline/token', authenticateCustomer, verifyCustomerMatch, paymentLimiter, auditLog('GENERATE_PAYMENT_TOKEN'), [
...
app.get('/api/customers', async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT id, customer_id, phone, name, email, address, pan_card, business_id,
             terms_accepted, terms_accepted_at, terms_accepted_ip, is_active,
             registration_date, total_invested, total_gold, transaction_count,
             last_transaction, created_at, updated_at,
             (SELECT COUNT(*) FROM schemes s WHERE s.customer_phone = c.phone AND s.status = 'ACTIVE') as total_schemes
      FROM customers c
      ORDER BY created_at DESC
    `);
...
// Deactivate/Activate customer (soft delete)
app.patch('/api/customers/:phone/toggle-active', async (req, res) => {
  try {
    const { phone } = req.params;
    console.log('🔄 Admin: Toggling active status for customer:', phone);

    const request = pool.request();
    request.input('phone', sql.NVarChar(15), phone);

    // Get current status
    const currentStatus = await request.query(`
      SELECT is_active FROM customers WHERE phone = @phone
    `);

    if (currentStatus.recordset.length === 0) {
      return res.status(404).json({ success: false, message: 'Customer not found' });
    }

    const newStatus = !currentStatus.recordset[0].is_active;

    // Toggle status
    const updateRequest = pool.request();
    updateRequest.input('phone', sql.NVarChar(15), phone);
    updateRequest.input('is_active', sql.Bit, newStatus);
    ...
    console.log(`✅ Customer ${phone} ${newStatus ? 'activated' : 'deactivated'}`);
    res.json({
      success: true,
...
// Permanently delete customer and all associated data
app.delete('/api/customers/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    console.log('🗑️ Admin: Permanently deleting customer:', phone);
    ...
    try {
      ...
      const deleteTransactionsRequest = new sql.Request(transaction);
      deleteTransactionsRequest.input('phone', sql.NVarChar(15), phone);
      const transactionsResult = await deleteTransactionsRequest.query(`
        DELETE FROM transactions WHERE customer_phone = @phone
      `);
      console.log(`   Deleted ${transactionsResult.rowsAffected[0]} transactions`);

      // 2. Delete all schemes
      const deleteSchemesRequest = new sql.Request(transaction);
      deleteSchemesRequest.input('phone', sql.NVarChar(15), phone);
      const schemesResult = await deleteSchemesRequest.query(`
        DELETE FROM schemes WHERE customer_phone = @phone
      `);
      console.log(`   Deleted ${schemesResult.rowsAffected[0]} schemes`);

      // 3. Delete customer record
      const deleteCustomerRequest = new sql.Request(transaction);
      deleteCustomerRequest.input('phone', sql.NVarChar(15), phone);
      const customerResult = await deleteCustomerRequest.query(`
        DELETE FROM customers WHERE phone = @phone
      `);

      if (customerResult.rowsAffected[0] === 0) {
        await transaction.rollback();
        return res.status(404).json({ success: false, message: 'Customer not found' });
      }

      await transaction.commit();
      console.log(`✅ Customer ${phone} and all associated data permanently deleted`);

      res.json({
        success: true,
        message: 'Customer and all associated data deleted successfully',
        deleted: {
          transactions: transactionsResult.rowsAffected[0],
          schemes: schemesResult.rowsAffected[0]
        }
      });
  ...
  try {
    ...
    const transactionsResult = await transactionsRequest.query(`
      SELECT transaction_id, type, amount, gold_grams, silver_grams,
             ...
             status, gateway_transaction_id, scheme_id, timestamp, created_at
      FROM transactions
      WHERE customer_phone = @phone
      ORDER BY timestamp DESC
    `);
...
app.get('/api/transactions/:transaction_id', async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT 
          t.id, t.transaction_id, t.customer_phone, t.amount, t.status, 
          t.payment_method, t.created_at, t.updated_at,
          t.business_id, t.scheme_id, t.gold_grams, t.silver_grams,
          t.gold_price_per_gram, t.silver_price_per_gram,
          t.gateway_transaction_id, t.gateway_response,
          c.customer_id,
          c.name as customer_name,
          c.email as customer_email
        FROM transactions t
...
app.post('/api/admin/notifications/send', authenticateAdmin, auditLog('SEND_NOTIFICATION'), [
...
app.post('/api/admin/notifications/broadcast', authenticateAdmin, auditLog('BROADCAST_NOTIFICATION'), [
  body('type').notEmpty().withMessage('Notification type required'),
  body('title').notEmpty().withMessage('Title required'),
  body('message').notEmpty().withMessage('Message required'),
], async (req, res) => {
...
app.post('/api/admin/notifications/send-filtered', authenticateAdmin, auditLog('SEND_FILTERED_NOTIFICATION'), [
...
app.put('/api/notifications/:notification_id/read', authenticateCustomer, auditLog('READ_NOTIFICATION'), async (req, res) => {
  try {
  ...
  try {
    ...
    httpsServer.on('error', (error) => {
      ...
      if (error.message.includes('asn1 encoding routines') || error.message.includes('ASN1')) {
        ...
        console.error(`1. Stop any existing server on port ${PORT}`);
        console.error(`2. Check: sudo netstat -tuln | grep ${PORT}`);
        console.error('3. Restart server');
      } else {
        console.error('');
        console.error('🔧 Common fixes:');
        console.error('1. Check SSL certificates are valid');
        console.error(`2. Ensure port ${PORT} is not in use`);
    ...
    httpsServer.listen(httpsPort, '0.0.0.0', () => {
      ...
      console.log(`🏥 Health Check: https://${MASTER_DOMAIN}/health`);
      console.log(`🔗 API Base URL: https://${MASTER_DOMAIN}/api`);
      console.log(`💾 Database: ${sqlConfig.database} on ${sqlConfig.server}`);
      console.log(`🔒 Privacy & Legal URLs:`);
      console.log(`   Privacy:  https://${MASTER_DOMAIN}/privacy-policy`);
      console.log(`   Terms:    https://${MASTER_DOMAIN}/terms-of-service`);
      console.log(`   Account Deletion: https://${MASTER_DOMAIN}/account-deletion`);
      console.log(`💳 Worldline Payment Integration (Clean Slate Rebuild):`);
      console.log(`   Token:    https://${MASTER_DOMAIN}/api/payments/worldline/token`);
      console.log(`   Verify:   https://${MASTER_DOMAIN}/api/payments/worldline/verify`);
      console.log(`💳 Omniware UPI Payment Integration (UPI Mode):`);
      console.log(`   Payment Page URL: https://${MASTER_DOMAIN}/api/omniware/payment-page-url`);
      console.log(`   Payment Status:   https://${MASTER_DOMAIN}/api/omniware/check-payment-status`);
      console.log(`   Webhook:          https://${MASTER_DOMAIN}/api/omniware/webhook/payment`);
      console.log('');
...
// Get all schemes (for admin portal)
app.get('/api/admin/schemes', authenticateAdmin, async (req, res) => {
  try {
    console.log('🎯 Admin: Getting all schemes...');
    const result = await pool.request().query(`
      SELECT 
        s.id, s.scheme_id, s.scheme_type, s.metal_type, s.monthly_amount, 
  ...
  try {
    ...
    const transactionsResult = await pool.request().query(`
      SELECT 
        ...
        SUM(CASE WHEN CAST(created_at AS DATE) = CAST(GETDATE() AS DATE) THEN gold_grams ELSE 0 END) as gold_sold_today,
        SUM(CASE WHEN CAST(created_at AS DATE) = CAST(GETDATE() AS DATE) THEN silver_grams ELSE 0 END) as silver_sold_today,
        SUM(CASE WHEN metal_type = 'GOLD' OR (metal_type IS NULL AND gold_grams > 0) THEN amount ELSE 0 END) as total_gold_revenue,
        SUM(CASE WHEN metal_type = 'SILVER' OR (metal_type IS NULL AND silver_grams > 0) THEN amount ELSE 0 END) as total_silver_revenue,
        ...
        SUM(CASE WHEN (CAST(created_at AS DATE) >= DATEADD(MONTH, -1, GETDATE())) AND (metal_type = 'GOLD' OR (metal_type IS NULL AND gold_grams > 0)) THEN amount ELSE 0 END) as gold_revenue_this_month,
        SUM(CASE WHEN (CAST(created_at AS DATE) >= DATEADD(MONTH, -1, GETDATE())) AND (metal_type = 'SILVER' OR (metal_type IS NULL AND silver_grams > 0)) THEN amount ELSE 0 END) as silver_revenue_this_month
      FROM transactions
...
// Scheme-wise Report
app.get('/api/admin/reports/scheme-wise', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    if (start_date && end_date) {
      ...
      if (end_date.length <= 10) end.setHours(23, 59, 59, 999);

      request.input('start_date', sql.DateTime, start);
      request.input('end_date', sql.DateTime, end);
      dateFilter = 'AND created_at >= @start_date AND created_at <= @end_date';
    }

    // If dates are provided, we look for schemes with transaction activity during that period
    // Otherwise, we show all schemes
    const query = `
      SELECT
        s.scheme_type,
        s.metal_type,
        COUNT(DISTINCT s.scheme_id) as total_schemes,
        COUNT(DISTINCT CASE WHEN s.status = 'ACTIVE' THEN s.scheme_id END) as active_count,
        COUNT(DISTINCT CASE WHEN s.status = 'COMPLETED' THEN s.scheme_id END) as completed_count,
        COUNT(DISTINCT CASE WHEN s.status = 'CANCELLED' THEN s.scheme_id END) as cancelled_count,
        COUNT(DISTINCT CASE WHEN s.status = 'PAUSED' THEN s.scheme_id END) as paused_count,
        SUM(ISNULL(t_stats.period_investment, 0)) as total_investment,
        SUM(ISNULL(t_stats.period_gold, 0)) as total_gold_accumulated,
        SUM(ISNULL(t_stats.period_silver, 0)) as total_silver_accumulated,
        AVG(s.monthly_amount) as avg_monthly_amount,
        AVG(CAST(s.completed_installments AS DECIMAL(10,2))) as avg_completed_installments,
        MIN(s.monthly_amount) as min_monthly_amount,
        MAX(s.monthly_amount) as max_monthly_amount
      FROM schemes s
      LEFT JOIN (
        SELECT 
          scheme_id, 
          SUM(amount) as period_investment,
          SUM(gold_grams) as period_gold,
          SUM(silver_grams) as period_silver
        FROM transactions
        WHERE status = 'SUCCESS' ${start_date && end_date ? 'AND created_at >= @start_date AND created_at <= @end_date' : ''}
        GROUP BY scheme_id
      ) t_stats ON s.scheme_id = t_stats.scheme_id
      WHERE 1=1 ${start_date && end_date ? 'AND t_stats.period_investment IS NOT NULL' : ''}
      GROUP BY s.scheme_type, s.metal_type
      ORDER BY s.scheme_type
    `;
...
app.get('/api/admin/reports/customer-wise', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    if (start_date && end_date) {
      ...
      transactionsRequest.input('start_date', sql.DateTime, start);
      transactionsRequest.input('end_date', sql.DateTime, end);
      dateFilter = 'AND created_at >= @start_date AND created_at <= @end_date';
    }
...
app.get('/api/admin/reports/transaction-wise', authenticateAdmin, async (req, res) => {
...
// Date-wise Report
app.get('/api/admin/reports/date-wise', authenticateAdmin, async (req, res) => {
...
// Month-wise Report
app.get('/api/admin/reports/month-wise', authenticateAdmin, async (req, res) => {
...
// Year-wise Report
app.get('/api/admin/reports/year-wise', authenticateAdmin, async (req, res) => {
 ...
 */
app.post('/api/payments/omniware/initiate', authenticateCustomer, verifyCustomerMatch, paymentLimiter, auditLog('INITIATE_OMNIWARE_PAYMENT'), [
...
// Bank Settlement Report API
app.get('/api/admin/reports/settlement', authenticateAdmin, async (req, res) => {
  const { start_date, end_date } = req.query;
  try {
    const pool = await sql.connect(sqlConfig);
    const request = pool.request();
    ...
    transactions.forEach(t => {
      ...
      batches[batchKey].total_amount += parseFloat(t.amount);
      batches[batchKey].transaction_count++;
